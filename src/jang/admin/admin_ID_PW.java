...
public class admin_ID_PW extends JFrame implements ActionListener, MouseListener {
	...
	public admin_ID_PW(String title, int width, int height) {
		setTitle(title);
	...
	public static void main(String[] args) {
		JFrame j = new JFrame();
		db.JDBC.init();
		new admin_ID_PW("아이디/비번 찾기",350,200);
	}
	...
	private String lookid(String name) {
		String sql = "SELECT ID FROM ADMIN WHERE USERNAME= '"+name+"'";
	...
	private String lookPw(String name) {
		String sql = "SELECT PW FROM ADMIN WHERE ID= '"+name+"'";
		ResultSet rs = db.JDBC.getResultSet(sql);
		String pw ="";
		int index =1;
		try {
	...
	private boolean checkID(String id, String pw2) {
		boolean check = false;
		String sql = "SELECT * FROM ADMIN WHERE  USERNAME='"+id+"'"+" AND PHONE='"+pw2+"'";
		ResultSet rs = db.JDBC.getResultSet(sql);
		try {
			if(rs.next()) {
			check = true;
	...
	private boolean checkPW(String id, String pw2) {
		boolean check = false;
		String sql = "SELECT * FROM ADMIN WHERE  ID='"+id+"'"+" AND PHONE='"+pw2+"'";
		ResultSet rs = db.JDBC.getResultSet(sql);
		try {
			if(rs.next()) {
			check = true;
